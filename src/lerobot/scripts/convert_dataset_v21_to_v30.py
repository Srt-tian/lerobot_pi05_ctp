...
def convert_videos(
    root: Path, new_root: Path, video_file_size_in_mb: int, preserve_episode_videos: bool = False
):
    ...
    for camera in video_keys:
        eps_metadata = convert_videos_of_camera(
            root, new_root, camera, video_file_size_in_mb, preserve_episode_videos
        )
...
def convert_videos_of_camera(
    ...
    preserve_episode_videos: bool = False,
):
    # Access old paths to mp4
    videos_dir = root / "videos"
    ep_paths = sorted(videos_dir.glob(f"*/{video_key}/*.mp4"))

    if preserve_episode_videos:
        metadata = []
        for ep_idx, ep_path in enumerate(ep_paths):
            chunk_idx, file_idx = divmod(ep_idx, DEFAULT_CHUNK_SIZE)
            target = new_root / DEFAULT_VIDEO_PATH.format(
                video_key=video_key, chunk_index=chunk_idx, file_index=file_idx
            )
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(ep_path, target)
            metadata.append(
                {
                    "episode_index": ep_idx,
                    ...
                    f"videos/{video_key}/to_timestamp": get_video_duration_in_s(ep_path),
                }
            )
        return metadata
...
def convert_dataset(
    ...
    episodes_videos_metadata = convert_videos(root, new_root, video_file_size_in_mb, preserve_episode_videos)
