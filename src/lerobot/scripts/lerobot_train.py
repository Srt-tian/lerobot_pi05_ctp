...
from lerobot.common.ctp_training import pin_best_model, stratified_indices
from lerobot.common.ctp_validation import (
    PaddedValidationDataset,
    color_jitter,
    evaluate_ctp,
    restore_rank_rng,
    selection_score,
...
def _preprocess_dataset_batch(
    ...
    if "action_is_pad" in batch:
        processed["action_is_pad"] = batch["action_is_pad"].to(processed["action"].device)
    return processed
...
def make_dataloaders(
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        if getattr(cfg.policy, "ctp_split_head", False)
        else None,
    ...
    if eval_dataset is not None:
        ...
        if cfg.eval_samples_per_episode > 0:
            episodes = eval_dataset.hf_dataset.data.column("episode_index").to_numpy()
            frames = eval_dataset.hf_dataset.data.column("frame_index").to_numpy()
            selected = stratified_indices(episodes, frames, cfg.eval_samples_per_episode)
            if len(selected) % (cfg.batch_size * parallel_dims.dp_world_size):
                raise ValueError("Balanced evaluation must divide evenly across global batches")
            eval_ds = torch.utils.data.Subset(eval_dataset, selected)
            if is_main_process():
                manifest = {
                    "episodes": sorted(set(episodes.tolist())),
                    "samples": len(selected),
                    "selection": [{"episode": int(episodes[i]), "frame": int(frames[i])} for i in selected],
                }
                (cfg.output_dir / "validation_selection.json").write_text(json.dumps(manifest, indent=2))
        ...
        if getattr(cfg.policy, "ctp_split_head", False):
            eval_ds = PaddedValidationDataset(eval_ds, cfg.batch_size * parallel_dims.dp_world_size)
...
def train(cfg: TrainPipelineConfig):
    ...
    if cfg.full_eval_steps:
        if not getattr(cfg.policy, "ctp_split_head", False) or eval_dataset is None:
            raise ValueError("Full CTP validation requires a V3 policy and held-out data")
        full_eval_dataloader = torch.utils.data.DataLoader(
            PaddedValidationDataset(eval_dataset, cfg.batch_size * parallel_dims.dp_world_size),
            batch_size=cfg.batch_size,
            shuffle=False,
            num_workers=cfg.num_workers,
        ...
        full_eval_dataloader = accelerator.prepare(full_eval_dataloader)
    ...
    loop_end = cfg.stop_after_step or cfg.steps
    if loop_end <= step:
        raise ValueError("stop_after_step must exceed the restored step; clear it for formal resume")
    ...
    if cfg.resume and getattr(cfg.policy, "ctp_split_head", False):
        restore_rank_rng(cfg.checkpoint_path / TRAINING_STATE_DIR, accelerator)
    for _ in range(step, loop_end):
        step_start = time.perf_counter()
        batch = next(dl_iter)
        preprocessing_start = time.perf_counter()
        if hasattr(accelerator.unwrap_model(policy), "set_training_step"):
            accelerator.unwrap_model(policy).set_training_step(step)
        if cfg.ctp_color_jitter:
            batch = color_jitter(
                batch,
                dataset.meta.camera_keys,
                cfg.ctp_color_jitter,
                (cfg.seed or 0) + 1009 * (step * parallel_dims.dp_world_size + accelerator.process_index),
            )
        ...
        is_saving_step = should_save_checkpoint(step, cfg.save_freq, loop_end)
        is_env_eval_step = cfg.env_eval_freq > 0 and step % cfg.env_eval_freq == 0
        is_eval_step = (
            cfg.eval_steps > 0
            and eval_dataloader is not None
            and (step % cfg.eval_steps == 0 or step == loop_end)
        )
                ...
                if wandb_logger:
                    ...
                    for group in optimizer.param_groups:
                        if "group_name" in group:
                            wandb_log_dict[f"lr_{group['group_name']}"] = group["lr"]
        ...
        if is_eval_step and getattr(cfg.policy, "ctp_split_head", False):
            episodes = sorted(set(eval_dataset.hf_dataset.data.column("episode_index").to_pylist()))

            def preprocess_validation(raw):
                return _preprocess_dataset_batch(raw, dataset.meta.camera_keys, cfg.rename_map, preprocessor)

            eval_metrics_for_checkpoint = evaluate_ctp(
                policy,
                eval_dataloader,
                accelerator,
                preprocess_validation,
                episodes,
                eval_dataloader.dataset.real_size,
            )
            eval_metrics_for_checkpoint["ctp_selection_score"] = selection_score(eval_metrics_for_checkpoint)
            if is_main_process():
                logging.info(f"step {step}: heldout_metrics={eval_metrics_for_checkpoint}")
                with (cfg.output_dir / "validation_history.jsonl").open("a") as stream:
                    stream.write(json.dumps({"step": step, **eval_metrics_for_checkpoint}) + "\n")
                if wandb_logger:
                    wandb_logger.log_dict(eval_metrics_for_checkpoint, step=step, mode="eval")
            if full_eval_dataloader is not None and (step % cfg.full_eval_steps == 0 or step == loop_end):
                full_metrics = evaluate_ctp(
                    policy,
                ...
                if is_main_process():
                    with (cfg.output_dir / "full_validation_history.jsonl").open("a") as stream:
                        stream.write(json.dumps({"step": step, **full_metrics}) + "\n")
                    if wandb_logger:
                        wandb_logger.log_dict(full_metrics, step=step, mode="full_eval")
            ...
            with torch.no_grad(), accelerator.autocast():
                for eval_batch in eval_dataloader:
                    ...
                    loss, diagnostics = policy(eval_batch)  # FSDP2 hooks must run
                    values = {
                        "eval_loss": loss.item(),
                        **{key: value for key, value in diagnostics.items() if key.startswith("ctp_")},
                    }
                    count = len(eval_batch["action"])
                    valid_steps = (
                        (~eval_batch["action_is_pad"]).sum().item()
                        if "action_is_pad" in eval_batch
                        else count
                    )
                    for key, value in values.items():
                        weight = valid_steps if key == "ctp_top1_mse" else count
                        totals[key] = totals.get(key, 0.0) + float(value) * weight
                        weights[key] = weights.get(key, 0.0) + weight
            keys = sorted(totals)
            packed = torch.tensor([[totals[k], weights[k]] for k in keys], device=device, dtype=torch.float64)
            packed = accelerator.reduce(packed, reduction="sum").cpu().tolist()
            eval_metrics_for_checkpoint = {k: s / n for k, (s, n) in zip(keys, packed, strict=True)}
            policy.train()
        ...
        if cfg.save_checkpoint and is_saving_step:
            ...
            if is_main_process():
                ...
                from lerobot.common.checkpoint_retention import mark_complete_and_prune

                mark_complete_and_prune(checkpoint_dir, cfg.checkpoint_keep_last)
                if cfg.best_checkpoint_metric and is_eval_step:
                    improved = pin_best_model(
                        checkpoint_dir, eval_metrics_for_checkpoint, cfg.best_checkpoint_metric
                    )
                    if improved:
                        logging.info(f"New best model at step {step}: {cfg.best_checkpoint_metric}")
                        if wandb_logger:
                            wandb_logger.log_dict(
                                {
                                    "best_step": step,
                                    "best_metric": eval_metrics_for_checkpoint[cfg.best_checkpoint_metric],
                                },
                                step=step,
                                mode="eval",
                            )
