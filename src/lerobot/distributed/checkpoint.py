...
def load_sharded_optimizer(
    ...
    if getattr(getattr(model, "config", None), "ctp_enabled", False):
        from torch.distributed.checkpoint.state_dict import StateDictOptions

        plugin = _fsdp_plugin(accelerator)
        if plugin.fsdp_version != 2:
            raise RuntimeError("CTP lazy optimizer recovery requires FSDP2")
        # These output layers are retained for base-checkpoint compatibility but
        # do not participate in the CTP forward. Adam never creates their slots.
        allowed_empty = {
        ...
        # The expert consumes the final VLM layer's K/V, not its output
        # projection/MLP or the terminal language norm. Those terminal outputs
        # likewise never reach the action loss in the joint attention graph.
        language_model = model.model.paligemma_with_expert.paligemma.model.language_model
        last_layer = len(language_model.layers) - 1
        prefix = f"model.paligemma_with_expert.paligemma.model.language_model.layers.{last_layer}"
        allowed_empty.update(
            {
                ...
                "model.paligemma_with_expert.paligemma.model.language_model.norm.weight",
            }
        )
        options = StateDictOptions(
            full_state_dict=False,
            cpu_offload=getattr(plugin.state_dict_config, "offload_to_cpu", False),
        )
        accelerator.wait_for_everyone()
        load_optimizer_with_lazy_states(
            model, optimizer, input_dir / f"{OPTIMIZER_NAME}_0", options, allowed_empty
        )
        accelerator.wait_for_everyone()
        return
...
def load_optimizer_with_lazy_states(model, optimizer, checkpoint_dir, options, allowed_empty):
    """Restore Adam slots strictly while retaining known unused parameters' empty states.

    PyTorch creates slots for all parameters when constructing a fresh load
    template. Only an explicitly allowed parameter with NO saved slot is changed
    back to an empty state. Partial slots and missing active parameters still
    fail through DCP's strict planner; parameter groups remain unchanged.
    """
    import logging
    ...
    state = get_optimizer_state_dict(model, optimizer, options=options)
    empty_names = []
    for name in allowed_empty:
        prefix = f"optimizer.state.{name}."
        if name in state["state"] and not any(k.startswith(prefix) for k in metadata.state_dict_metadata):
            state["state"][name] = {}
            empty_names.append(name)
    dcp.load({"optimizer": state}, storage_reader=reader)
    set_optimizer_state_dict(model, optimizer, state, options=options)
    logging.info(
        "Restored optimizer; retained %d known unused parameters without Adam slots", len(empty_names)
    )
