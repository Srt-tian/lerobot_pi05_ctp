...
def update_last_checkpoint(checkpoint_dir: Path) -> None:
    ...
    temporary = checkpoint_dir.parent / f".{LAST_CHECKPOINT_LINK}.{os.getpid()}.tmp"
    try:
        temporary.symlink_to(relative_target)
        os.replace(temporary, last_checkpoint_dir)
    finally:
...
def save_checkpoint(
    ...
    """
    if cfg.checkpoint_min_free_gb > 0 or cfg.checkpoint_keep_last > 0:
        import torch.distributed as dist

        from lerobot.common.checkpoint_retention import check_space

        error = [None]
        if is_main_process():
            try:
                # Conservative upper bound for weights and two Adam moments in FP32.
                expected = sum(p.numel() * max(4, p.element_size()) * 3 for p in policy.parameters())
                check_space(checkpoint_dir.parent, expected, cfg.checkpoint_min_free_gb)
...
def save_training_state(
    ...
    *,
    ...
    if getattr(cfg.policy, "ctp_split_head", False):
        from lerobot.common.ctp_validation import save_rank_rng

        save_rank_rng(save_dir, accelerator)
