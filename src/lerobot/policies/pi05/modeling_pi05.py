...
from .ctp_head import SplitCTPHead, responsibility_temperature
from .ctp_loss import conditional_trajectory_peak_loss
from .ctp_mode_belief import TrajectoryMatchedModeBelief
...
class PI05Pytorch(nn.Module):  # see openpi `PI0Pytorch`
    ...
    def __init__(self, config: PI05Config, rtc_processor: RTCProcessor | None = None):
        ...
        self.ctp_parameter_head = None
        self.ctp_mode_selector: TrajectoryMatchedModeBelief | None = None
        self.ctp_action_dim = int(config.output_features[ACTION].shape[0])
        self.ctp_parameters_per_mode = config.chunk_size * self.ctp_action_dim + 2
        if config.ctp_enabled:
            if config.ctp_split_head:
                self.ctp_parameter_head = SplitCTPHead(
                    action_expert_config.width,
                    config.ctp_num_modes,
                    config.chunk_size * self.ctp_action_dim,
                    config.ctp_head_init_std,
                    config.ctp_min_width,
                    config.ctp_initial_width,
                    config.ctp_max_width,
                    config.ctp_detach_width_context,
                )
            else:
                self.ctp_parameter_head = nn.Linear(
                    action_expert_config.width,
                    config.ctp_num_modes * self.ctp_parameters_per_mode,
                )
                nn.init.normal_(self.ctp_parameter_head.weight, mean=0.0, std=config.ctp_head_init_std)
                nn.init.zeros_(self.ctp_parameter_head.bias)
                width_fraction = (config.ctp_initial_width - config.ctp_min_width) / (
                    config.ctp_max_width - config.ctp_min_width
                )
                width_bias = math.log(width_fraction / (1.0 - width_fraction))
                with torch.no_grad():
                    shaped_bias = self.ctp_parameter_head.bias.reshape(
                        config.ctp_num_modes, self.ctp_parameters_per_mode
                    )
                    shaped_bias[:, -1] = width_bias
            self.ctp_mode_selector = TrajectoryMatchedModeBelief(
                inference_mode=config.ctp_inference_mode,
                execute_horizon=config.n_action_steps,
                cumulative_action_dims=config.ctp_belief_cumulative_action_dims,
                self_transition=config.ctp_belief_self_transition,
                observation_weight=config.ctp_belief_observation_weight,
                release_ratio=config.ctp_belief_release_ratio,
                transport_temperature=config.ctp_belief_transport_temperature,
                confidence_threshold=config.ctp_belief_confidence_threshold,
                probability_floor=config.ctp_belief_probability_floor,
            )

        self.last_ctp_probabilities: Tensor | None = None
        self.last_ctp_posterior: Tensor | None = None
        self.last_ctp_widths: Tensor | None = None
        self.last_ctp_selected_modes: Tensor | None = None
        self.last_ctp_matched_continuation: Tensor | None = None
    ...
    def ctp_parameters(
        ...
        """Predict all CTP trajectory centers, masses, and widths in one pass."""
        if self.ctp_parameter_head is None:
            raise RuntimeError("CTP parameters requested while ctp_enabled=False.")

        batch_size = tokens.shape[0]
        device = tokens.device
        # Zero action tokens are learned query slots. They contain no sampled
        # noise and are processed once, so CTP inference has NFE=1.
        query_actions = torch.zeros(
            batch_size,
        ...
        query_time = torch.zeros(batch_size, dtype=torch.float32, device=device)
        prefix_embs, prefix_pad_masks, prefix_att_masks = self.embed_prefix(images, img_masks, tokens, masks)
        suffix_embs, suffix_pad_masks, suffix_att_masks, adarms_cond = self.embed_suffix(
            query_actions, query_time
        )
        ...
        att_2d_masks_4d = prepare_attention_masks_4d(make_att_2d_masks(pad_masks, att_masks))
        position_ids = torch.cumsum(pad_masks, dim=1) - 1

        def forward_func(prefix_embs, suffix_embs, attention_mask, position_ids, adarms_cond):
        ...
        trajectory_context = suffix_out[:, -self.config.chunk_size :].float().mean(dim=1)
        raw = (
            self.ctp_parameter_head(trajectory_context)
            .float()
            .reshape(batch_size, self.config.ctp_num_modes, self.ctp_parameters_per_mode)
        )
        trajectory_size = self.config.chunk_size * self.ctp_action_dim
        trajectories = raw[..., :trajectory_size].reshape(
            batch_size,
            self.config.ctp_num_modes,
            self.config.chunk_size,
            self.ctp_action_dim,
        )
        logits = raw[..., trajectory_size]
        widths = self.config.ctp_min_width + (
            self.config.ctp_max_width - self.config.ctp_min_width
        ) * torch.sigmoid(raw[..., trajectory_size + 1])
        self.last_ctp_probabilities = torch.softmax(logits.detach(), dim=-1)
        self.last_ctp_widths = widths.detach()
        return trajectories, logits, widths

    @torch.no_grad()
    def sample_ctp_actions(
        self,
        images: list[Tensor],
        img_masks: list[Tensor],
        tokens: Tensor,
        ...
        """Select one complete trajectory from a single CTP backbone call."""
        trajectories, logits, _ = self.ctp_parameters(images, img_masks, tokens, masks)
        if self.ctp_mode_selector is None:
            raise RuntimeError("CTP selector requested while ctp_enabled=False.")
        selection = self.ctp_mode_selector.select(logits, trajectories)
        selected_modes = selection.selected
        self.last_ctp_probabilities = selection.probabilities.detach()
        self.last_ctp_posterior = selection.posterior.detach()
        self.last_ctp_selected_modes = selected_modes.detach()
        self.last_ctp_matched_continuation = selection.matched_continuation.detach()
        batch_index = torch.arange(trajectories.shape[0], device=trajectories.device)
        return trajectories[batch_index, selected_modes]

    def reset_ctp_state(self) -> None:
        """Clear episode-local CTP correspondence state without erasing diagnostics."""
        if self.ctp_mode_selector is not None:
            self.ctp_mode_selector.reset_episode()

    def get_ctp_diagnostics(self) -> dict[str, float | int | str]:
        if self.ctp_mode_selector is None:
            return {}
        return self.ctp_mode_selector.summary()
...
class PI05Policy(PreTrainedPolicy):
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    "Pretrained weights were not loaded; refusing random initialization"
                ) from e
            ...
            if config.ctp_enabled:
                ...
                invalid_missing = [k for k in missing_keys if not k.startswith("model.ctp_parameter_head.")]
                if invalid_missing or unexpected_keys:
                    raise RuntimeError(
                        f"Pretrained mismatch: missing={invalid_missing}, unexpected={unexpected_keys}"
                    )
            else:
            ...
            raise RuntimeError("Failed to load the requested Pi0.5 checkpoint") from e
    ...
    def get_optim_params(self):
        if self.config.ctp_enabled and (
            self.config.ctp_split_head
            or self.config.ctp_optimizer_vlm_lr_scale != 1.0
            or self.config.ctp_optimizer_head_lr_scale != 1.0
        ):
            ...
            return parameter_groups(
                ...
                self.config.ctp_optimizer_vlm_lr_scale,
                self.config.ctp_optimizer_head_lr_scale,
                logits_scale=self.config.ctp_optimizer_logits_lr_scale
                if self.config.ctp_split_head
                else None,
                width_scale=self.config.ctp_optimizer_width_lr_scale if self.config.ctp_split_head else None,
            )
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor], **kwargs: Unpack[ActionSelectKwargs]) -> Tensor:
        ...
        if self.config.ctp_enabled:
            actions = self.model.sample_ctp_actions(images, img_masks, tokens, masks)
        else:
    ...
    def set_training_step(self, step: int):
        self._ctp_training_step = int(step)

    def forward(
        self, batch: dict[str, Tensor], reduction: str = "mean", return_per_sample_metrics: bool = False
    ) -> tuple[Tensor, dict]:
        ...
        if self.config.ctp_enabled:
            ...
            action_is_pad = batch["action_is_pad"] if self.config.ctp_mask_padding else None
            trajectories, logits, widths = self.model.ctp_parameters(images, img_masks, tokens, masks)
            per_sample_loss, ctp_metrics = conditional_trajectory_peak_loss(
                trajectories,
                logits,
                widths,
                target_actions,
                overlap_weight=self.config.ctp_overlap_weight,
                probability_floor=self.config.ctp_probability_floor,
                entropy_target_effective_modes=self.config.ctp_entropy_target_effective_modes,
                entropy_weight=self.config.ctp_entropy_weight,
                overlap_probability_weighted=self.config.ctp_overlap_probability_weighted,
                overlap_detach_widths=self.config.ctp_overlap_detach_widths,
                overlap_detach_probabilities=self.config.ctp_overlap_detach_probabilities,
                temperature=responsibility_temperature(
                    getattr(self, "_ctp_training_step", 0),
                    self.config.ctp_temperature_initial,
                    self.config.ctp_temperature_final,
                    self.config.ctp_temperature_anneal_steps,
                )
                if self.training
                else 1.0,
            ...
            if return_per_sample_metrics:
                return per_sample_loss, {name: value.detach() for name, value in ctp_metrics.items()}
            loss_dict = {
                f"ctp_{name}": value.detach().float().mean().item() for name, value in ctp_metrics.items()
            }
