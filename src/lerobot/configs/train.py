...
class TrainPipelineConfig(HubMixin):
    ...
    steps: int = 100_000
    # Operational debug stop; preserves the full scheduler horizon in `steps`.
    stop_after_step: int = 0
    eval_samples_per_episode: int = 0
    best_checkpoint_metric: str | None = None
    full_eval_steps: int = 0
    ctp_color_jitter: float = 0.0
    ...
    checkpoint_min_free_gb: float = 10.0
    ...
    def validate(self) -> None:
        if not 0 <= self.stop_after_step <= self.steps:
            raise ValueError("stop_after_step must be 0 or at most steps")
        if self.eval_samples_per_episode < 0:
            raise ValueError("eval_samples_per_episode must be nonnegative")
        if self.best_checkpoint_metric and (
            self.eval_steps <= 0 or self.save_freq <= 0 or self.save_freq % self.eval_steps != 0
        ):
            raise ValueError(
                "Best model selection requires save_freq to be a positive multiple of eval_steps"
            )
        if self.checkpoint_keep_last < 0:
            raise ValueError("checkpoint_keep_last must be nonnegative")
        if not 0 <= self.ctp_color_jitter <= 0.2:
            raise ValueError("CTP color jitter must be in [0, .2]")
        if self.full_eval_steps < 0 or (
            self.full_eval_steps and (self.eval_steps <= 0 or self.full_eval_steps % self.eval_steps)
        ):
            raise ValueError("Full validation must be a positive multiple of eval_steps")
